...
int main() {
    ...
    PGresult *res = PQexec(conn, "COPY barcodes (id, code, val) FROM STDIN WITH (FORMAT text);");
    check_status(conn, res, PGRES_COPY_IN);

    const struct SourceRow sr1 = {123, "w2e3", "u8y7", 0};
    const struct SourceRow sr2 = {12345, "w2kke3", "u8y7xx", 0};
    const struct SourceRow sr3 = {1234, "ffw2e3", "nnu8y7", 0};
    struct Batch b;
    b.size = 3;
    b.rows[0] = sr1;
    b.rows[1] = sr2;
